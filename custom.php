...
$q = mysqli_query( $conn, $sql );
    ...
    echo '<h1>ประเพณีและวัฒนธรรม : '.$f['Ethnic_nameth'].' ( '.$f['Ethnic_nameen'].')'.'</h1>';
}

$sql2 = "SELECT * from custom c join ethnicdata e on e.Ethnic_id = c.Ethnic_id WHERE img='$n2'";
$q2 = mysqli_query( $conn, $sql2 );
    echo '<h2>รายละเอียดประเพณี : </h2>';
    while ($f2 = mysqli_fetch_assoc($q2)){
    echo '<h3>➢&nbsp'.$f2['Custom_name'].'</h3>'.$f2['Custom_detail'].'<br></br>⏲ &nbspช่วงเวลาจัดประเพณี : '
    .$f2['Custom_date'];
}
...
<a href="custommap.php"><input class ="button button2" type="button" name= "custommap" value="แผนที่"></a>
<script>
        function goBack() {
            window.history.back();
        }
