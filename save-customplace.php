...
if(!isset($_SESSION["Email"])){
    ...
    foreach($_POST['like'] as $value){
        ...
        $check_query = "SELECT * FROM keeplocation k JOIN customplace cup ON k.Customplace_id = cup.Customplace_id JOIN custom cu on cu.Custom_id = cup.Custom_id
        WHERE k.Customplace_id = '$value'and k.Email='$Email'";
        $query = mysqli_query($conn, $check_query);
        $result = mysqli_fetch_assoc($query);

        $Email = mysqli_real_escape_string($conn, $_SESSION["Email"]);
        $data = mysqli_real_escape_string($conn,$value);
          if (isset($result['Customplace_id'])){
            $message = $result['Custom_name'].'มีอยู่ในรายการสิ่งที่น่าสนใจของคุณอยู่แล้ว';
            echo "<script type='text/javascript'>alert('$message');</script>";
              }
          else{
        $sql = "INSERT INTO keeplocation (Email, Customplace_id) VALUES ('$Email', '$data')";
       $result2 = mysqli_query($conn, $sql) or die ("Error in query: $sql " . mysqli_error());
       
       }
    }
    
    $all = "SELECT * FROM keeplocation k join customplace cus on k.Customplace_id = cus.Customplace_id join custom cu on cu.Custom_id = cus.Custom_id
    where k.Email='$Email'";
    $q = mysqli_query( $conn, $all);
    echo "<h2>รายการประเพณีที่คุณสนใจ :</h2><br>";
    while ($f = mysqli_fetch_assoc($q)){
    echo '<h3><img class="img" src="heart.gif" style=width:40px;height:40px; >&nbsp'.$f['Custom_name']."</h3><br>";
    //echo $f['Ethnicplace_detail']."<br>";
}
