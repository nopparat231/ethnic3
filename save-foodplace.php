...
if(!isset($_SESSION["Email"])){
    ...
    foreach($_POST['like'] as $value){
        ...
        $check_query = "SELECT * FROM keeplocation k join foodplace fp on k.Foodplace_id = fp.Foodplace_id WHERE k.Foodplace_id = '$value'and k.Email='$Email' ";
        $query = mysqli_query($conn, $check_query);
        $result = mysqli_fetch_assoc($query);

        $Email = mysqli_real_escape_string($conn, $_SESSION["Email"]);
        $data = mysqli_real_escape_string($conn,$value);
          if (isset($result['Foodplace_id'])){
            $message = $result['Foodplace_name'].'มีอยู่ในรายการบันทึกสถานที่ที่สนใจของคุณอยู่แล้ว';
            echo "<script type='text/javascript'>alert('$message');</script>";
              }
          else{
        $sql = "INSERT INTO keeplocation (Email, Foodplace_id) VALUES ('$Email', '$data')";
       $result2 = mysqli_query($conn, $sql) or die ("Error in query: $sql " . mysqli_error());
       
       }
    }
    echo"<br><br>";
    $all = "SELECT k.Foodplace_id, fp.Foodplace_name FROM keeplocation k join foodplace fp
    on k.Foodplace_id = fp.Foodplace_id where k.Email='$Email'";
    $q = mysqli_query( $conn, $all);
    echo "<h2>รายการร้านอาหารที่คุณสนใจ :</h2><br>";
    while ($f = mysqli_fetch_assoc($q)){
    echo '<h3><img class="img" src="heart.gif" style=width:40px;height:40px; >&nbsp'.$f['Foodplace_name']."</h3><br>";
}
