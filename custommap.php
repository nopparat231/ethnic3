...
<script type="text/javascript" src="https://api.longdo.com/map/?key=7e3736b93742855c4af09070fffd2498"></script>
  ...
  /*session_start();*/
   ...
   while ($row = $ti->fetch_assoc()) {
    ...
    echo "  <h1>แหล่งประเพณีของชาติพันธุ์&nbsp:&nbsp$name</h1>";
   }
...
<?php
  $sql = "SELECT DISTINCT cup.Province_id, p.Province_name FROM customplace cup join province p on cup.Province_id = p.Province_id 
  join custom cus on cup.Custom_id = cus.Custom_id join ethnicdata e on  cus.Ethnic_id = e.Ethnic_id where e.img ='$n2' order by p.Province_name";
  ...
  while ($f = mysqli_fetch_assoc($q)){
   ...
   $sql2 = "SELECT *  FROM customplace cusp join custom c on cusp.Custom_id = c.Custom_id join ethnicdata e on e.Ethnic_id = c.Ethnic_id where cusp.Province_id='$id'and e.img='$n2' order by c.Custom_name";
   $q2 = mysqli_query( $conn, $sql2 );
   while ($f2 = mysqli_fetch_assoc($q2)){
    $name = $f2['Custom_name'];
    $id2 = $f2['Customplace_id'];
echo "<br>";
echo "<form action='save-customplace.php' method='post' name='savelocation'>";
