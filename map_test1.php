...
<?php
  ...
  <div id="map"></div>
<?php
  $sql = "SELECT DISTINCT ep.Province_id, p.Province_name FROM ethnicplace ep join province p on ep.Province_id = p.Province_id order by p.Province_name";
  ...
  while ($f = mysqli_fetch_assoc($q)){
   ...
   $sql2 = "SELECT *  FROM ethnicplace where Province_id='$id'order by Ethnicplace_detail";
   $q2 = mysqli_query( $conn, $sql2 );
   while ($f2 = mysqli_fetch_assoc($q2)){
    $name = $f2['Ethnicplace_detail'];
    $id2 = $f2['Ethnicplace_id'];
echo "<br>";
echo "<h5><input type='checkbox' name='like' value='btu-like' id='$id2'><label for='$id2'></label></h5><h4>".$name."</h4>";
