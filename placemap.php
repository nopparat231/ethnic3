...
<script type="text/javascript" src="https://api.longdo.com/map/?key=7e3736b93742855c4af09070fffd2498"></script>
  <script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js"></script>
      <script>
        ...
        function selectLocation(){
          ...
          detail: 'เวลาเปิด-ปิด: '+open+'-'+close+' น.',
  ...
  /*session_start();*/
   ...
   while ($row = $ti->fetch_assoc()) {
    ...
    echo "  <h1>สถานที่ท่องเที่ยวและพิพิธภัณฑ์ของชาติพันธุ์&nbsp:&nbsp$name</h1>";
   }
...
<?php
  ...
  $sql = "SELECT DISTINCT pl.Province_id, p.Province_name FROM place pl join province p on pl.Province_id = p.Province_id join ethnicdata e 
  on  pl.Ethnic_id = e.Ethnic_id where e.img ='$n2' order by p.Province_name";
  ...
  while ($f = mysqli_fetch_assoc($q)){
   ...
   $sql2 = "SELECT *  FROM place pl join ethnicdata e on pl.Ethnic_id = e.Ethnic_id where pl.Province_id='$id'and e.img='$n2' order by Location_name";
   $q2 = mysqli_query( $conn, $sql2 );
   while ($f2 = mysqli_fetch_assoc($q2)){
    $name = $f2['Location_name'];
    $id2 = $f2['Location_id'];
echo "<br>";
echo "<form action='save-place.php' method='post' name='savelocation'>";
