...
if(!isset($_SESSION["Email"])){
    ...
    foreach($_POST['like'] as $value){
        ...
        $check_query = "SELECT * FROM keeplocation k join clothesplace ep on k.Clothesplace_id = ep.Clothesplace_id WHERE k.Clothesplace_id = '$value'and k.Email='$Email' ";
        $query = mysqli_query($conn, $check_query);
        $result = mysqli_fetch_assoc($query);

        $Email = mysqli_real_escape_string($conn, $_SESSION["Email"]);
        $data = mysqli_real_escape_string($conn,$value);
          if (isset($result['Clothesplace_id'])){
            $message = $result['Clothesplace_name'].'มีอยู่ในรายการบันทึกสถานที่ที่สนใจของคุณอยู่แล้ว';
            echo "<script type='text/javascript'>alert('$message');</script>";
              }
          else{
        $sql = "INSERT INTO keeplocation (Email, Clothesplace_id) VALUES ('$Email', '$data')";
       $result2 = mysqli_query($conn, $sql) or die ("Error in query: $sql " . mysqli_error());
       
       }
    }
    echo"<br><br>";
    $all = "SELECT k.Clothesplace_id, ep.Clothesplace_name FROM keeplocation k join Clothesplace ep
    on k.Clothesplace_id = ep.Clothesplace_id where k.Email='$Email'";
    $q = mysqli_query( $conn, $all);
    echo "<h2>รายการร้านขายเสื้อผ้าที่คุณสนใจ :</h2><br>";
    while ($f = mysqli_fetch_assoc($q)){
    echo '<h3><img class="img" src="heart.gif" style=width:40px;height:40px; >&nbsp'.$f['Clothesplace_name']."</h3><br>";
}
