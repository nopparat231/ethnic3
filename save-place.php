...
if(!isset($_SESSION["Email"])){
    ...
    foreach($_POST['like'] as $value){
        ...
        $check_query = "SELECT * FROM keeplocation JOIN place ON keeplocation.Location_id = place.Location_id WHERE keeplocation.Location_id = '$value'and keeplocation.Email='$Email' ";
        ...
        $data = mysqli_real_escape_string($conn,$value);
          if (isset($result['Location_id'])){
            $message = $result['Location_name'].'มีอยู่ในรายการสถานที่ที่สนใจของคุณอยู่แล้ว';
            echo "<script type='text/javascript'>alert('$message');</script>";
              }
          else{
        $sql = "INSERT INTO keeplocation (Email, Location_id) VALUES ('$Email', '$data')";
       $result2 = mysqli_query($conn, $sql) or die ("Error in query: $sql " . mysqli_error());
       
       }
    }
    
    $all = "SELECT * FROM keeplocation k join place pl on k.Location_id = pl.Location_id 
    where k.Email='$Email'";
    $q = mysqli_query( $conn, $all);
    echo "<h2>รายการแหล่งท่องเที่ยวและพิพิธภัณฑ์ที่คุณสนใจ :</h2><br>";
    while ($f = mysqli_fetch_assoc($q)){
    echo '<h3><img class="img" src="heart.gif" style=width:40px;height:40px; >&nbsp'.$f['Location_name']."</h3><br>";
